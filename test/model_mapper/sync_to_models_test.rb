...
class ModelMapperSyncToModelsTest < Minitest::Test
  ...
  def test_that_sync_to_models_returns_form_itself
    assert_same(@team, @team.sync_to_models(default: @team_model, chassis: @chassis_model))
  end

  def test_that_all_attributes_value_are_correctly_synced_from_empty_form_into_empty_models
    check_that_all_attributes_value_are_correctly_synced_from_empty_form
    check_that_not_synced_attributes_are_still_empty
  end

  def test_that_all_attributes_value_are_correctly_synced_from_empty_form_into_filled_models
    fill_in_models
  ...
  def test_that_all_attributes_value_are_correctly_synced_from_filled_form_into_empty_models
    fill_in_form
  ...
  def test_that_all_attributes_value_are_correctly_synced_from_filled_form_into_filled_models
    fill_in_form
    fill_in_models
  ...
  def check_that_all_attributes_value_are_correctly_synced_from_empty_form
    @team.sync_to_models(default: @team_model, chassis: @chassis_model)
    ...
    assert_kind_of(::Array, @chassis_model.chassis)
    assert_equal(0, @chassis_model.chassis.size)
  ...
  def check_that_all_attributes_value_are_correctly_synced_from_filled_form
    ...
    assert_kind_of(::Array, @chassis_model.chassis)
    assert_equal(2, @chassis_model.chassis.size)

    assert_equal(@team.chassis[0].id, @chassis_model.chassis[0][:id])
    assert_equal(@team.chassis[0].suspension.front, @chassis_model.chassis[0][:suspension].front)
    assert_equal(@team.chassis[0].suspension.rear, @chassis_model.chassis[0][:suspension].rear)
    assert_equal(@team.chassis[0].brakes, @chassis_model.chassis[0][:brakes])

    assert_equal(@team.chassis[1].id, @chassis_model.chassis[1][:id])
    assert_equal(@team.chassis[1].suspension.front, @chassis_model.chassis[1][:suspension].front)
    assert_equal(@team.chassis[1].suspension.rear, @chassis_model.chassis[1][:suspension].rear)
    assert_equal(@team.chassis[1].brakes, @chassis_model.chassis[1][:brakes])
