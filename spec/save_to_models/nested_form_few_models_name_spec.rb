RSpec.describe FormObj, concept: true do
  describe 'SaveToModel: nested form - few models - name' do
    ...
    describe 'nested form' do
          ...
          module FewModelsName
            class Form < FormObj
              ...
              attribute :car, hash: true do
                attribute :model
                attribute :engine, model_class: 'SaveToModel::NestedForm::FewModelsName::Engine' do
                  attribute :power
                  attribute :volume
                end
                attribute :driver
              ...
              attribute :chassis, model_attribute: false, model: :chassis do
                attribute :suspension, model_class: 'SaveToModel::NestedForm::FewModelsName::Suspension' do
                  attribute :front
                  attribute :rear
                end
                attribute :brakes
      ...
      context 'nested models are created when they do not exist yet' do
        ...
        let(:form) { SaveToModel::NestedForm::FewModelsName::Form.new }
      ...
      context 'nested models are updated when they exists already' do
        before do
          model.car = { engine: SaveToModel::NestedForm::FewModelsName::Engine.new }
          chassis.suspension = SaveToModel::NestedForm::FewModelsName::Suspension.new
    ...
    describe 'explicit declaration of nested form classes' do
        ...
        module NestedForm
          module FewModelsName
            ...
            class CarForm < FormObj
              attribute :model
              attribute :engine, class: EngineForm, model_class: 'SaveToModel::NestedForm::FewModelsName::Engine'
              attribute :driver
            end
      ...
      context 'nested models are created when they do not exist yet' do
        ...
        let(:form) { SaveToModel::NestedForm::FewModelsName::TeamForm.new }
